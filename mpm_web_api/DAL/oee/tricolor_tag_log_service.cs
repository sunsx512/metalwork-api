...
namespace mpm_web_api.DAL.oee
{
    ...
    public class tricolor_tag_log_service : SqlSugarBase
    {
        ...
        public List<tricolor_tag_log> GetList<T>(int machine_id) where T : new()
        {
            var query = DB.Queryable<tricolor_tag_log>().Where(x => x.machine_id == machine_id).ToList();
            return query;
        ...
        public bool update<T>(tricolor_tag_log obj) where T : class, new()
        {
        ...
        {
            return DB.Deleteable<tricolor_tag_log>(id).ExecuteCommand() > 0;
        }
        ...
        public bool Delete<T>(string timespan)
        {
            ...
            try
            {
                 res= DB.Deleteable<tricolor_tag_log>(it => it.insert_time < DateTime.Now).ExecuteCommand();
            }
            catch(Exception ex)
            {
