...
{
    public class utilization_rate_shift_service: SqlSugarBase
    {
        //根据设备id ，更新稼动率和时间
        // public bool update<T>(decimal utilization_rate, int machine_id) where T : class, new()
        public bool update<T>(utilization_rate_shift obj) where T : class, new()
        {
        ...
        //按字段查询
        public List<utilization_rate_shift> GetList<T>(int machine_id) where T : new()
        {
            var query = DB.Queryable<utilization_rate_shift>().Where(x => x.machine_id == machine_id).ToList();
            return query;
        }
